...
getwd()
Hb2<-read_csv("Quarterly_Data_Analysis_Reviewed3.csv")
View(Hb2)
str(Hb2)


Year<-as.factor(Hb2$Year)
names(Hb2)
Hb2$date<-as.Date(Hb2$Date, "%d/%m/%Y")
Hb2$date
Endemicity<-as.factor(Hb2$Endemicity)

Prev1<-as.numeric(Hb2$Prev1)


#Hb2<-drop_na(Hb2)

Hb4 <- Hb2 %>%
  group_by(Hb2$ID, Hb2$Hb_1C,Hb2$Prev1,Hb2$date) %>%
  summarise(count=n())

View(Hb4)
#For Cases

p <- ggplot(Hb2, aes(x =Hb2$date))
...
p<-p+geom_smooth(aes(y=Hb), 
                 method="auto",se = T,
                 col="blue")
p<-p+facet_wrap(~Endemicity, ncol = 5)
...
p2 <- ggplot(Hb2, aes(x =Hb2$date))
p2 <- p2 + geom_point(aes(y = Hb2$Prev1*100)) +labs(x = "YEAR", y = "Prevalence (%)", caption = "(DHIS2 data, https://hiskenya.org)") + ylim(0,100) + theme_bw() 

p2<-p2+geom_smooth(aes(y=Prev1*100), 
                 method="auto",se = T,
                 col="red")
p2<-p2+facet_wrap(~Endemicity, ncol = 5)

show(p2)

mean(Hb2$Prev1)

######Both Prevalence and cases

pp <- ggplot(Hb2, aes(x = date, Hb_1C))
pp <- pp + geom_point(aes(y = Hb2$Hb_1C))
pp<-pp+geom_smooth(aes(y=Hb_1C, colour="Hb cases"), 
                 method="auto",se = F) 
names(Hb2)

pp <- pp + geom_point(aes(x=date, y = Prev1*3000))
pp<-pp+geom_smooth(aes(y=Prev1*3000, colour = "Prevalence"),
                 method="loess",se = T)
pp <- pp + scale_y_continuous(sec.axis = sec_axis(~./30, name = "Prevalence (%)"))

pp <- pp + theme(legend.position = c(0.1, 0.5))

pp <- pp + scale_colour_manual("", values = c("blue", "red")) + theme(legend.position = c(0.1, 0.5)) + theme_classic()
pp <- pp + labs(y = "Reported cases (Hb<11g/dl)",
              x = "YEAR")

pp <- pp + labs(y = "Reported cases (Hb<11g/dl)",
              x = "YEAR") 
pp <- pp + theme(legend.position = c(0.09, 0.92)) 

show(pp)

#####################

p1<- ggplot(Hb2, aes(x = Hb2$date, Hb_1C))
p1 <- p1 + geom_point(aes(y = Hb2$Hb_1C), na.rm = TRUE) +ylim(0,3000) + labs(x = "YEAR", y = "Estimated Cases (Hb<11g/dl)", caption = "(DHIS2 data, https://hiskenya.org)") + theme_bw() 

p1<-p1+geom_smooth(aes(y=Hb_1C), 
                 method="loess",se = T,
                 col="blue") 
p1<-p1+facet_wrap(~Endemicity, ncol = 5) 

p1 <- p1 + geom_point(aes(x=Hb2$date, y = Prev1*3000), na.rm = TRUE) + theme_bw()

p1<-p1+geom_smooth(aes(x=Hb2$date, y=Prev1*3000, colour = "Prevalence"),
                 method="loess",se = T, col ="red")
p1 <- p1 + scale_y_continuous(sec.axis = sec_axis(~./30, name = "Prevalence (%)"))


p1 <- p1 + labs(colour = "Metric",
                y = "Reported cases (Hb<11g/dl)",
                x = "YEAR")
p1 <- p1 + scale_colour_manual("", values = c("red"))  + theme(legend.position = c(0.9, 0.9), legend.justification = c("right", "top")) + theme_grey()
...
figure<- (ggarrange(pp, p1, ncol = 1, labels = c("A", "B")))  # Label of the line plot
...
######################################
#BOX PLOT FOR REPORTED CASES - 47 COUNTIES
Year<-as.factor(Hb_data$year)
names(Hb2)

Hb2$Prev2<-as.numeric(Hb2$Prev)
...
p <- p + geom_point(aes(x=date, y = Prev2*10000))
p<-p+geom_smooth(aes(y=Prev2*10000, colour = "Prevalence"),
                 method="loess",se = T)
...
p2 <- ggplot(Hb2, aes(factor(Hb2$Quarter), Prev1*100, colour = "Endemicity")) +labs(x = "YEAR", y = "Prevalence (%)", title = "Prevalence, 2016 - 2019") +  theme_grey() 
...
# plot
g <- ggplot(Hb2, aes(x= Hb2$date, y = Hb2$Hb))
g + geom_boxplot() + 
