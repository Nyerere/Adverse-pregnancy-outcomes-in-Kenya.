...
Hb_data$Date<-as.Date(Hb_data$Period, "%d/%m/%Y")
Hb_data$Date
...
names(Hb2)
Hb2$date<-as.Date(Hb2$Period, "%d/%m/%Y")
Hb2$date
...
p1 <- p1 + geom_point(aes(x=Days, y = Vload*200))
p1<-p1+geom_line(aes(x = Days, y=Vload*200, color="Viral Load"))
p1 <- p1 + scale_y_continuous(sec.axis = sec_axis(~./200, name = "Viral load"))
p1<-p1+scale_color_manual(values = c("blue","red"));
p1<-p1+geom_vline(xintercept = 0, linetype="dotted", 
                  color = "green", size=1)

p1 <- p1 + labs(y = "CD4 Count",
                x = "No of Days",
                colour = "Measure")
